  	        ...
  	        function(e) {
  	            ...
  	            if ($('#tree1').tree('isNodeSelected', selected_node)) {
  	        		...
  	        		for(var i=0;i<nodes.length-1;i++){
  	        			idArrStr=idArrStr+nodes[i].id+";";
  	        		}
  	        		idArrStr=idArrStr+nodes[nodes.length-1].id;
  	            	$('#labelSelectedKnowId').text(idArrStr);
  	            }
  	        }
  	    );
...
//	    '}' +
//	  ']';
		alert('getTree'+$('#selectForSubject').val());
		$.getJSON('/admin/questionAdmin/getKnow', {
			subjectId:$('#selectForSubject').val(),
			gradeId:-1,
			areaId:-1,
...
//	//复制id到剪贴板上
//	$('#btnCopyKnowId').click(function(){
//		alert('clip');
//		$("#btnCopyKnowId").zclip({
//	        path:'/plugins/zeroClipBoard/ZeroClipboard.swf', //记得把ZeroClipboard.swf引入到项目中 
//	        copy:function(){
//	            return $('#labelSelectedKnowId').text();
//	        }
//	    });
//	});
